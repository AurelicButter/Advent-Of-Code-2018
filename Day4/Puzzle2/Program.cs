...
namespace Puzzle2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<string> guardKeys = new List<string>(guards.Keys);
            string guardID = null;
            int min = 0;
            int amount = 0;

            guardID = guards[guardKeys[0]].ID;
            min = guards[guardKeys[0]].commonMin;
            amount = guards[guardKeys[0]].commonMinTotal;

            for (int x = 0; x < guardKeys.Count; x++) {
                if (guards[guardKeys[x]].commonMinTotal > amount) {
                    guardID = guards[guardKeys[x]].ID;
                    min = guards[guardKeys[x]].commonMin;
                    amount = guards[guardKeys[x]].commonMinTotal;
                }
            }

            Console.WriteLine("Guard ID: " + guardID + "\nMinute: " + min + "\nAmount: " + amount);
            Console.WriteLine("Answer: " + Convert.ToInt32(guardID) * min);
            Console.ReadLine();
        }
    }
}
